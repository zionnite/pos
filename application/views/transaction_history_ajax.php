...
<table  class="table table-striped footable footable-1 footable-paging footable-paging-center breakpoint-lg" style="" id="dataItem">
	<thead>
		<tr class="footable-header">
			...
			<th class="footable-sortable footable-first-visible" style="display: table-cell;" style="width:100px;">Product Name
			...
			<th class="footable-sortable footable-first-visible" style="display: table-cell;">Sub total
				<span class="fooicon fooicon-sort"></span></th>
	...
	<tbody>
		...
		<tr>
			...
			<td style="width:100px;"><?php echo $prod_name;?></td>
					...
					swal({
						...
						.then((willDelete) => {
							if (willDelete) {
								...
								$.ajax({
									type: 'POST',
									url: '<?php echo base_url();?>Transaction_history/delete_transaction/' +
										id,

									success: function (resp) {
										if (resp == 'ok') {

											$('#dataItem').load('<?php echo base_url();?>Transaction_history/index_2');
											swal({
												title: "Success",
												text: "Transaction has been been removed from List!",
												icon: "success",
...
<?php 
    if(!is_array($get_info)){
?>
    <div class="alert alert-warning" style="margin-top:1%;">No Transaction Record found!</div>
<?php
    }
?>
